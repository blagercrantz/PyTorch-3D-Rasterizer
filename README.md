...
3. In get_depth_planes(), the depth planes (ax + by + cz + d = 0) are calculated for every surface, where x and y are the "2D coordinates" of the corners, as displayed on the screen, and where z is the reciprocal of depth. It is the reciprocal of depth, and not depth, since the reciprocal varies linearly across the screen, and depth itself does not. The depth planes will be useful for displaying intersecting surface where neither surface is fully in front of the other.
